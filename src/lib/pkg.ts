 ...
 */
export async function getPackage(owner: string, repository: string, path: string = "package.json"): Promise<z.infer<typeof PACKAGE_JSON_SCHEMA>> {
  ...
  if (
    ...
    throw new Error(
      "invalid github api response for package.json",
    );
  }
